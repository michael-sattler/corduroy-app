...
import { loadStaffPlanDashboard } from "@/lib/plan/load-staff-plan-dashboard";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await loadStaffPlanDashboard(supabase, clientId);
    ...
    const message =
      error instanceof Error ? error.message : "Staff plan dashboard query failed";
