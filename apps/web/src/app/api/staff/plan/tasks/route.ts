...
import { loadStaffPlanTasks } from "@/lib/plan/load-staff-plan-tasks";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await loadStaffPlanTasks(supabase, clientId);
    ...
    const message =
      error instanceof Error ? error.message : "Staff plan tasks query failed";
