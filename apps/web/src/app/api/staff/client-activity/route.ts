...
import { loadStaffClientActivity } from "@/lib/staff-client-activity";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const activity = await loadStaffClientActivity(supabase, clientId);
    ...
    const message =
      error instanceof Error ? error.message : "Client activity query failed";
