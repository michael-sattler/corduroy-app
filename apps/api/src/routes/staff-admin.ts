...
function userDb(config: ApiConfig, request: FastifyRequest) {
...
export async function registerStaffAdminRoutes(
  app: FastifyInstance,
  config: ApiConfig,
) {
  await app.register(
    async (admin) => {
      ...
      admin.get("/waitlist", async (request) => {
        ...
        const { data, error } = await db
          ...
          .select(
            "id, name, company, email, status, submitted_at, updated_at",
          )
      ...
      admin.get<{ Params: { clientId: string } }>(
        "/clients/:clientId",
        async (request, reply) => {
          const db = userDb(config, request);
          const { data, error } = await db
