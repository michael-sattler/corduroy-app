export async function checkOrchestrationApi(apiBaseUrl) {
    ...
    try {
        ...
        return {
            service: "Orchestration API",
            status: "down",
            detail: error instanceof Error ? error.message : "Orchestration API unreachable",
            latencyMs: null,
            checkedAt,
        };
    }
}
export async function checkSupabase(supabaseUrl, anonKey, accessToken) {
    const started = Date.now();
    const checkedAt = new Date().toISOString();
    try {
...
export function checkNotConfigured(service, milestone) {
    return {
        service,
        status: "degraded",
